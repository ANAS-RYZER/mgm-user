...
interface DashboardHomeProps {
  onNavigate: (
    tab: "home" | "custom-request" | "wishlist" | "appointments" | "settings",
  ) => void;
  profile: any;
}
...
const DashboardHome = ({
  ...
  if (isFetching) return <AnimatedPage isLoading={true} />;
  return (
    <div className=" px-2 py-4 md:p-6  lg:p-8 space-y-8">
      {/* Welcome Section */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-mgm p-8 md:mt-0 lg:mt-0 mt-6">
        <div className="relative z-10">
          ...
          <p className="text-primary-foreground/90 max-w-xl">
            Explore our exquisite collection, request custom jewelry, and manage
      ...
      {/* Quick Actions */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:p-0 px-2">
        ...
        {[
          {
            ...
            value: dashboard?.data?.wishlistCount || 0,
            icon: Heart,
          },
          {
            label: "Appointments",
            value: dashboard?.data?.totalAppointments || 0,
            icon: Calendar,
          },
          {
            label: "Store Visits",
            value: dashboard?.data?.storeVisitCount || 0,
            icon: TrendingUp,
          },
        ].map((stat, index) => (
          <motion.div

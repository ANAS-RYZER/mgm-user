  ...
  const WishlistButton = ({ productId }: { productId: string | number }) => (
    <Button
      onClick={(e) => {
        e.stopPropagation();
        e.preventDefault();
        wishlist.toggle(Number(productId));
      }}
      className={`p-2 rounded-lg transition-colors h-10 w-10 hover:bg-black/10 text-black border shadow-[0_4px_12px_rgba(0,0,0,0.06)] ${wishlist.isIn(Number(productId)) ? "bg-destructive text-white" : "bg-background/90 text-red-500"}`}
